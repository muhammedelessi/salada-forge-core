...
export default function ShopPage() {
  ...
  const formatPrice = (price: number) =>
    new Intl.NumberFormat('ar-SA', { style: 'currency', currency: 'SAR', minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(price);
  ...
  const handleCategoryChange = (id: string) => {
    if (id) { searchParams.set('category', id); } else { searchParams.delete('category'); }
    setSearchParams(searchParams);
  };

  const handlePriceRangeChange = (id: string) => {
    if (id && id !== priceRange) { searchParams.set('price', id); } else { searchParams.delete('price'); }
    setSearchParams(searchParams);
  };
  ...
  const FilterContent = ({ onClose }: { onClose?: () => void }) => (
    <div dir={isAr ? 'rtl' : 'ltr'}>
      {/* Categories */}
      <p className="font-mono text-[0.53rem] uppercase tracking-[0.22em] mb-2.5 pb-2 border-b border-border"
        style={{ color: 'hsl(var(--muted-foreground))' }}>
        {t.shop.categories}
      </p>
      <ul className="space-y-0.5 mb-5">
        <li>
          <button onClick={() => { handleCategoryChange(''); onClose?.(); }}
        ...
        {categories.map(cat => (
          <li key={cat.id}>
            <button onClick={() => { handleCategoryChange(cat.id); onClose?.(); }}
              ...
              style={{
                ...
                borderInlineStart: activeCategory === cat.id ? '2px solid hsl(var(--primary))' : '2px solid transparent',
              }}>
              <span className="truncate">{categoryTranslations[cat.id] || cat.name}</span>
              <span className="font-mono text-[0.5rem] shrink-0 ms-2"
      ...
      <ul className="space-y-0.5">
        {priceRanges.map(range => (
          <li key={range.id}>
            <button onClick={() => { handlePriceRangeChange(range.id); onClose?.(); }}
              className="w-full text-start py-2 px-3 text-sm transition-colors duration-150"
              style={{
    ...
    <Layout>
      ...
      <section className="relative overflow-hidden" style={{ minHeight: '180px' }}>
        <div className="absolute inset-0">
          <img src={heroPort} alt={t.shop.catalog}
        ...
        <div className="industrial-container relative z-10 flex flex-col justify-center py-8 md:py-10"
          dir={isAr ? 'rtl' : 'ltr'} style={{ minHeight: '180px' }}>
          <div className="max-w-xl">
            <nav className="flex items-center gap-1.5 mb-3">
              <Link to="/" className="font-mono text-[0.46rem] uppercase tracking-[0.16em]"
                style={{ color: 'rgba(255,255,255,0.3)' }}>
                {isAr ? 'الرئيسية' : 'Home'}
              </Link>
              <span style={{ color: 'rgba(255,255,255,0.15)' }}>/</span>
              <span className="font-mono text-[0.46rem] uppercase tracking-[0.16em]"
            ...
            <h1 className="font-black uppercase leading-tight tracking-[-0.025em] mb-2"
            ...
            <div className="flex items-center gap-3 flex-wrap">
              {isLoading ? (
                <span className="inline-flex items-center gap-1.5 font-mono text-[0.58rem]"
      ...
      <div className="lg:hidden sticky top-0 z-30 border-b border-border"
        style={{ background: 'hsl(var(--background))' }}>

        {/* Category tabs — horizontal scroll */}
        <div className="overflow-x-auto scrollbar-none" dir={isAr ? 'rtl' : 'ltr'}>
          <div className="flex items-center gap-0 px-3 py-0 min-w-max">
            {/* All */}
            <button
        ...
        <div className="flex items-center gap-2 px-3 py-2 border-t border-border" dir={isAr ? 'rtl' : 'ltr'}>
          <div className="relative flex-1">
            ...
            <ChevronDown className="absolute end-2.5 top-1/2 -translate-y-1/2 w-3 h-3 pointer-events-none"
              style={{ color: 'hsl(var(--muted-foreground))' }} />
          </div>
          ...
          {activeCount > 0 && (
            <button onClick={clearFilters}
              className="shrink-0 p-2 border border-border transition-colors hover:border-primary"
              style={{ color: 'hsl(var(--primary))' }}>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="hidden lg:flex items-center justify-between gap-4 mb-6 pb-5 border-b border-border">
              <span className="font-mono text-[0.58rem] uppercase tracking-[0.15em]"
                style={{ color: 'hsl(var(--muted-foreground))' }}>
                {filteredProducts.length} {t.shop.products}
              </span>
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <ChevronDown className="absolute end-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 pointer-events-none"
                    style={{ color: 'hsl(var(--muted-foreground))' }} />
                </div>
            ...
            {isLoading ? (
              ...
              {/* Mobile: always list | Desktop: grid or list based on toggle */}
              <div className={
                viewMode === 'grid'
                  ? 'hidden lg:grid grid-cols-2 xl:grid-cols-3 gap-4'
                  : 'flex flex-col gap-3'
              }>
              ...
              {/* Mobile list — always shown on small screens */}
